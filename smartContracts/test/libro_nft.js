...
contract("LibroNFT", function (accounts) {
  ...
  */
  ...
  it('Account0 is librarianRole', async () => {
    const isBibliotecario = await libroNFT.isLibrarian();
    return assert(isBibliotecario);
  });

  it("Account1 isn't librarianRole", async () => {
    const isBibliotecario = await libroNFT.isLibrarian({ from: accounts[1] });
    return assert(!isBibliotecario);
  });

  it("Add new librarianRole", async () => {
    const isNotBibliotecarioStart = await libroNFT.isLibrarian({ from: accounts[1] });
    await libroNFT.addLibrarian(accounts[1]);
    const isBibliotecarioStart = await libroNFT.isLibrarian({ from: accounts[1] });
